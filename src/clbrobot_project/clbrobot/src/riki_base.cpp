    ...
    heading_(0)
{
    ...
    velocity_subscriber_ = nh_.subscribe("cmd_vel", 50, &RikiBase::velCallback, this);
    nh_private.getParam("linear_scale", linear_scale_);
    
    //ROS_INFO("linear_scale_: %f", linear_scale_);
}

void RikiBase::velCallback(const geometry_msgs::Twist& vel)
{
    ros::Time current_time = ros::Time::now();

    linear_velocity_x_ = vel.linear.x * linear_scale_;
    linear_velocity_y_ = vel.linear.y * linear_scale_;
